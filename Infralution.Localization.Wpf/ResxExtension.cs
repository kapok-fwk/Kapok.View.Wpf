...
namespace Infralution.Localization.Wpf
{
    ...
    public class ResxExtension : ManagedMarkupExtension
    {
            ...
            get 
            {
                ...
                if (string.IsNullOrEmpty(result))
                {
                    if (_defaultResxName == null)
                    {
                        ...
                        if (targetRef?.Target is DependencyObject o)
                        {
                            _defaultResxName = o.GetValue(DefaultResxNameProperty) as string;
                        }
                    }
        ...
        /// </summary>
        public Binding Binding => _binding ?? (_binding = new Binding());
        ...
        {
            get => _bindingTargetNullKey;
            set => _bindingTargetNullKey = value;
        }
        ...
        {
            foreach (var managedMarkupExtension in _markupManager.ActiveExtensions)
            {
                var ext = (ResxExtension) managedMarkupExtension;
                if (ext.Key == key)
                {
                    ext.UpdateTargets();
                }
        ...
        protected virtual object GetLocalizedResource(string resourceKey)
        {
            ...
            if (!string.IsNullOrEmpty(ResxName))
            {
                try
                {
                    if (GetResource != null)
                    {
                        result = GetResource(ResxName, resourceKey, CultureInfo.CurrentUICulture);
                    }
                    if (result == null)
                    {
                        ...
                        if (_resourceManager != null)
                        {
                            result = _resourceManager.GetObject(resourceKey, CultureInfo.CurrentUICulture);
                        }
                    }
                }
                catch
                {
                    ...
                    Debug.WriteLine(
                        _resourceManager == null
                            ? $"ResxExtension: Could not find resource key \"{resourceKey}\" with culture \"{CultureInfo.CurrentUICulture}\". Resource manager not found."
                            : $"ResxExtension: Could not find resource key \"{resourceKey}\" with culture \"{CultureInfo.CurrentUICulture}\" in resource manager \"{_resourceManager.BaseName}\".");
#endif
                }
            }
            return result;
        ...
        protected override void UpdateTarget(object target)
        {
            ...
            if (IsMultiBindingParent)
            {
                if (target is FrameworkElement el)
                {
                    ...
                    // use optimistic behaviour here
                    if (TargetProperty is DependencyProperty dependencyProperty)
                        el.SetBinding(dependencyProperty, multiBinding);
                }
            }
            else if (IsBindingExpression)
            {
                if (target is FrameworkElement el)
                {
                    ...
                    // use optimistic behaviour here
                    if (TargetProperty is DependencyProperty dependencyProperty)
                        el.SetBinding(dependencyProperty, binding);
                }
            }
            else
            {
        ...
        private object ConvertValue(object value)
        {
            ...
            if (targetType != null)
            {
                if (value is string s && targetType != typeof(String) && targetType != typeof(object))
                {
        ...
        private static void OnDefaultResxNamePropertyChanged(DependencyObject element, DependencyPropertyChangedEventArgs args)
        {
            ...
            {
                foreach (var managedMarkupExtension in MarkupManager.ActiveExtensions)
                {
                    var ext = (ResxExtension) managedMarkupExtension;
